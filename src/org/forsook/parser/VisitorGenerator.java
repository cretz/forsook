...
public class VisitorGenerator {
    ...
    public StringBuilder generate(Map<Class<?>, NavigableSet<ParseletInstance>> parselets) {
        ...
        Map<Class<?>, ClassInfo> classes = new TreeMap<Class<?>, ClassInfo>(classComparator);
        for (Class<?> clas : parselets.keySet()) {
            addClassInfo(clas, classes, Collections.EMPTY_MAP);
        }
        //imports
        ...
        Map<Class<?>, ClassInfo> newlyFoundClasses;
        Map<Class<?>, ClassInfo> overallClasses = new TreeMap<Class<?>, ClassInfo>(classComparator);
        overallClasses.putAll(classes);
        do {
            newlyFoundClasses = new TreeMap<Class<?>, ClassInfo>(classComparator);
            for (ClassInfo info : classes.values()) {
                ret.append(buildMethod(info, imports, overallClasses, newlyFoundClasses));
            }
            classes = newlyFoundClasses;
            overallClasses.putAll(newlyFoundClasses);
        } while (!newlyFoundClasses.isEmpty());
    ...
    private static void addClassInfo(Class<?> clas, Map<Class<?>, ClassInfo> classes,
            Map<Class<?>, ClassInfo> ignoreClasses) {
        ...
        while (parent != null) {
            if (!ignoreClasses.containsKey(parent)) {
        ...
        for (Class<?> iface : clas.getInterfaces()) {
            if (!ignoreClasses.containsKey(iface)) {
    ...
    private StringBuilder buildMethod(ClassInfo info, Set<String> imports,
            Map<Class<?>, ClassInfo> classes, Map<Class<?>, ClassInfo> newlyFoundClasses) {
            ...
            for (Method method : info.className.getMethods()) {
                if ((method.getName().startsWith("is") || 
                    ...
                    if (method.getReturnType() == List.class) {
                        ...
                        if (!classes.containsKey(listClass)) {
                            addClassInfo(listClass, newlyFoundClasses, classes);
                        }
                        imports.add("java.util.List");
                        ...
                        if (!classes.containsKey(method.getReturnType())) {
                            addClassInfo(method.getReturnType(), newlyFoundClasses, classes);
                        }
